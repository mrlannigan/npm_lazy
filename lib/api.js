...
api.get(new RegExp('^/([^/]+)$'), function(req, res, match) {
  var name = match[1];
    ...
    if (Git.mapping.hasOwnProperty(name)) {
        Git.getFullIndex(name, function (err, filecontent) {
            if (err) {
...
api.get(new RegExp('^/([^/]+)/(.+)/([^/]+)$'), function(req, res, match) {
  ...
  logger.log('cache get', uri);
    ...
    if (Git.mapping.hasOwnProperty(name)) {
        ...
        Git.getTarball(name, function (err, stream) {
            if (err) {
                res.statusCode = err.statusCode || 500;
                logger.error('[' + res.statusCode + '] Error: ', err);
                if (err.content) {
...
api.get(new RegExp('^/([^/]+)/([^/]+)$'), function(req, res, match) {
  var name = match[1],
    ...
    if (Git.mapping.hasOwnProperty(name)) {
        Git.getIndex(name, function (err, filecontent) {
            if (err) {
                res.statusCode = err.statusCode || 500;
                logger.error('[' + res.statusCode + '] Error: ', err);
                if (err.content) {
